...
if st.session_state["if_complete_onboarding"]:
    quiz = st.Page("pages/quiz.py", title="Quiz", icon=":material/quiz:", default=False, url_path="quiz")
...
if not st.session_state["if_complete_onboarding"]:
    ...
    pg = st.navigation({"GenMentor": [quiz, learner_profile, learning_path]}, position="hidden", expanded=False)
else:
    nav_position = "sidebar"
    pg = st.navigation({"GenMentor": [learner_profile, learning_path, quiz]}, position=nav_position, expanded=True)
    with st.sidebar:
